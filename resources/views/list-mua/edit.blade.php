<x-dashboard-layout>
  <div class="container mx-auto p-6">
    <h1 class="text-2xl font-bold mb-6">Edit Data MUA</h1>
    ...
    <form action="{{ route('list-mua.update', $mua->id_mua) }}" method="POST" class="bg-white p-6 rounded-lg shadow-md">
      @csrf
      @method('PUT') {{-- Use PUT method for update --}}

      <div class="mb-4">
        <label for="nama_mua" class="block text-gray-700 text-sm font-bold mb-2">Nama MUA:</label>
        <input type="text" name="nama_mua" id="nama_mua" value="{{ old('nama_mua', $mua->nama_mua) }}"
      ...
      <div class="mb-4">
        ...
        <input type="text" name="no_telp" id="no_telp" value="{{ old('no_telp', $mua->no_telp) }}"
          class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
          required>
      </div>
      ...
      <div class="flex items-center">
        ...
        <a href="{{ route('list-mua.index') }}" class="btn btn-neutral ml-2">
          Kembali
        </a>
      </div>
    </form>
