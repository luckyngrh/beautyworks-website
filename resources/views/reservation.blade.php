<x-layout>
  ...
  <div class="grid grid-cols-2 grid-rows-2 gap-4" id="mainGridContainer">
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    const mainGridContainer = document.getElementById('mainGridContainer'); // ADDED: Get main grid container
      ...
      fetch('/update-payment-status', { // Ganti dengan rute yang sesuai di web.php
          ...
          headers: {
            ...
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute(
              'content') // Pastikan ada meta csrf-token di layout Anda
          },
          body: JSON.stringify({
            order_id: orderId,
            status: status
    ...
    function displayAlert(paymentStatus) {
      ...
      if (message) {
        const alertDiv = `
                ...
                `;
        // MODIFIED: Insert alert before mainGridContainer
        if (mainGridContainer) { // Pastikan elemen ditemukan
          mainGridContainer.insertAdjacentHTML('beforebegin', alertDiv);
        } else {
          console.error(
            'Elemen #mainGridContainer tidak ditemukan. Alert tidak dapat ditampilkan di lokasi yang diinginkan.');
          // Fallback to inserting before the form if mainGridContainer is not found
          reservationForm.insertAdjacentHTML('beforebegin', alertDiv);
        }

        // MODIFIED: Adjust the way existingAlert is selected to ensure it's the one just inserted
        setTimeout(() => {
          // This assumes only one alert of this type is shown at a time right before mainGridContainer
          const existingAlert = mainGridContainer.previousElementSibling;
          if (existingAlert && (existingAlert.classList.contains('bg-green-100') || existingAlert.classList
              .contains('bg-yellow-100') || existingAlert.classList.contains('bg-red-100'))) {
